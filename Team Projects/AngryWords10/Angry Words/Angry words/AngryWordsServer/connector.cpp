...
int Connector::currentLevel(QString username)
{
...
{
    std::string query = "select username, currentLevel, playedTime, wordsHit, wordsMissed, longestWordHit, longestStreak from User;";
    mysql_query(connection, query.c_str());
    MYSQL_RES *result = mysql_store_result(connection);
    MYSQL_ROW row;
...
{
    std::string query = "SELECT username, currentLevel, playedTime, wordsHit, wordsMissed, longestWordHit, longestStreak FROM User, Teaches where student = username and teacher = '" + teacher.toStdString() + "';";
    mysql_query(connection, query.c_str());
    MYSQL_RES *result = mysql_store_result(connection);
    MYSQL_ROW row;
...
void Connector::updateStats(QString user, int currentLevel, int timePlayed, int wordsHit, int wordsMissed, QString longestWordHit, int longestStreak)
{
    //Should the checks happen here or in the server?

    std::vector<QString> stats = getStudentStats(user);

    std::string cLevel = QString::number(currentLevel).toStdString();
    ...
    std::string query = "update User set `currentLevel`='" + cLevel + "', `playedTime`='" + tPlayed + "', `wordsHit`='" + wHit + "', `wordsMissed`='" + wMissed + "', `longestWordHit`='" + lWord + "', `longestStreak`='" + lStreak + "' where `username`='" + user.toStdString() + "';";
    mysql_query(connection, query.c_str());
}
